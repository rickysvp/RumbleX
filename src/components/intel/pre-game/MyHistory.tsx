    ...
    <div className="p-5 border-b border-app-border">
      ...
      {!isConnected ? (
        <div className="py-8 px-4 border border-dashed border-[#222] bg-[#050505] flex flex-col items-center justify-center text-center">
          <div className="text-[10px] text-app-muted font-app-bold uppercase tracking-[2px] mb-4">Connection Required to view history</div>
          <button 
            onClick={mockWallet.connect}
            className="w-full bg-app-accent text-black font-app-bold text-[12px] py-3 uppercase tracking-wide hover:bg-white transition-colors"
          >
            Connect Wallet
          </button>
        </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {queueRemaining > 0 && Array.from({ length: Math.min(queueRemaining, 3) }).map((_, i) => (
               <div key={`queued-${i}`} className="grid grid-cols-[40px_60px_1fr_80px] gap-2 items-center py-1 opacity-50">
                 ...
                 <div className="text-[11px] font-app-bold text-right pr-2 text-[#666]">---</div>
            ...
            {userHistory.map((h) => (
              <div key={h.roundNumber} className="grid grid-cols-[40px_60px_1fr_80px] gap-2 items-center py-1">
                ...
                <div className={`text-[11px] font-app-bold text-right pr-2 ${h.monDelta >= 0 ? 'text-app-accent' : 'text-[#FF4444]'}`}>
            ...
            {userHistory.length === 0 && queueRemaining === 0 && (
               <div className="text-[10px] font-app-bold text-app-muted italic py-4">No recent activity.</div>
            )}
          </div>

          {userStats && (
            <div className="mt-6 flex justify-between items-center text-[10px] text-app-muted font-app-bold uppercase tracking-wide bg-[#050505] p-2 border border-[#1a1a1a]">
